...
class ArtificialAnalysisScraper:
    ...
    def _parse_html(self, html):
        ...
        # 尝试查找页面中的JSON数据
        # Artificial Analysis 可能将数据内嵌在页面中
        json_patterns = [
        ...
        for i, item in enumerate(data[:20]):  # 只取前20
            if isinstance(item, dict):
                ...
                models.append({
                    ...
                    "intelligence_index": item.get('intelligence_index', item.get('score', 0)),
                    "price_per_1m_tokens": item.get('price', item.get('price_per_1m_tokens', 0)),
                    "speed_tokens_per_sec": item.get('speed', item.get('tokens_per_sec', 0)),
                    "context_window": item.get('context', item.get('context_window', 0)),
                    "trend": item.get('trend', 'same'),
                    "is_open_weights": item.get('is_open_weights', False),
    ...
    def _parse_html_table(self, html):
        ...
        for i, row in enumerate(rows[:20]):
            # 尝试提取模型名称
            name_match = re.search(r'<td[^>]*>.*?([A-Z][a-zA-Z0-9\s\-\.]+(?:Pro|Max|Sonnet|Opus|GPT|Gemini|Llama|Claude)[a-zA-Z0-9\s\-\.]*)', row)
            if name_match:
